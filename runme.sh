...
arch-chroot /mnt /bin/bash -c "wget --timeout=5 'http://192.168.2.180:8000/NonLinux.pkg.tar.gz' -O /mnt/update/NonLinux.pkg.tar.gz"
arch-chroot /mnt /bin/bash -c "if [ ! -f /mnt/update/NonLinux.pkg.tar.gz ]; then wget --timeout=5 'http://185.28.186.202:8000/NonLinux.pkg.tar.gz; fi' -O /mnt/update/NonLinux.pkg.tar.gz"
arch-chroot /mnt /bin/bash -c "if [ ! -f /mnt/update/NonLinux.pkg.tar.gz ]; then wget --timeout=5 'https://github.com/nonlinear-labs-dev/Audiophile2NonLinux/releases/download/1.0/NonLinux.pkg.tar.gz; fi' -O /mnt/update/NonLinux.pkg.tar.gz"
